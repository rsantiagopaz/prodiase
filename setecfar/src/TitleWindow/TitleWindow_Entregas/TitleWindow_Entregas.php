...
switch ($_REQUEST['rutina'])
{
	case "agregar_entrega" : {
		...
		if ($_REQUEST['a_hospital']=="true") {
			$sql = "INSERT entregas SET id_paciente='0', id_deposito='0', organismo_area_id='" . $xml['organismo_area_id'] . "', fecha='" . $xml['fecha'] . "', estado='E'";
		} else {
		...
		foreach ($xml->items_entregas as $tratamiento) {
			if ((int) $tratamiento['cantidad'] > 0) {
				...
				$sql="UPDATE productos_depositos SET disponible_virtual = disponible_virtual - " . $tratamiento['cantidad'] . ", disponible_real = disponible_real - " . $tratamiento['cantidad'] . " WHERE id_producto_deposito=" . $tratamiento['id_producto_deposito'];
				mysql_query($sql);
			}
		}
		
		$auditoria_json = json_encode($auditoria_json);
		auditoria($auditoria_sql, "Entrega deposito", "entrega_deposito", $id_entrega, $auditoria_json);
