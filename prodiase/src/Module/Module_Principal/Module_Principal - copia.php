...
switch ($_REQUEST['rutina'])
{
	...
	case "agregar_entrega" : {
		...
		if (is_null($_REQUEST['id_financiador'])) {
			...
			$sql = "INSERT entregas SET id_paciente='" . $_REQUEST['id_paciente'] . "', id_deposito='0', organismo_area_id='', fecha = NOW(), estado = 'A', id_financiador='" . $_REQUEST['id_financiador'] . "'";
		}
...
function leer_datos_paciente($id_paciente) {
	...
	$sql = "SELECT DISTINCTROW id_financiador, nombre, siglas FROM suram.financiadores INNER JOIN suram.padrones USING(id_financiador) WHERE padrones.tipo_doc='" . $td . "' AND padrones.nrodoc='" . $row['persona_dni'] . "' ORDER BY siglas, nombre";
	...
	while ($rowFinanciadores = mysql_fetch_array($rsFinanciadores)) {
		...
		toXML($xmlObrasSociales, new SimpleXMLElement('<obra_social id_financiador="' . $rowFinanciadores['id_financiador'] . '" descrip="' . ((empty($rowFinanciadores['siglas'])) ? $rowFinanciadores['nombre'] : $rowFinanciadores['siglas'] . ", " . $rowFinanciadores['nombre']) . '"/>'));
	}
	toXML($nodo, $xmlObrasSociales);
	...
	$sql = "SELECT DISTINCTROW entregas.id_entrega, entregas.fecha, entregas.estado, entregas.id_financiador, productos_depositos.id_deposito";
	...
	while ($rowEntregas = mysql_fetch_array($rsEntregas)) {
		if (! is_null($rowEntregas['id_financiador'])) {
			...
			if (! is_null($rowFinanciadores['siglas'])) $rowFinanciadores['siglas'] = trim($rowFinanciadores['siglas']);
			$rowEntregas['obra_social_descrip'] = ((empty($rowFinanciadores['siglas'])) ? $rowFinanciadores['nombre'] : $rowFinanciadores['siglas'] . ", " . $rowFinanciadores['nombre']);
		}
