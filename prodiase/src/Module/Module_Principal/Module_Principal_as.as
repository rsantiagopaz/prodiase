...
private function btnNuevaEntrega_click(e:MouseEvent) : void {
	...
	twnEntrega.addEventListener("eventAceptar", function():void{
		...
		if (twnEntrega.cboObraSocial.selectedItem==null) {
			httpPaciente.send({rutina:"agregar_entrega", id_paciente: xmlModel.paciente.@id_paciente, model: twnEntrega.xmlModel.tratamientos});
		} else {
			httpPaciente.send({rutina:"agregar_entrega", id_paciente: xmlModel.paciente.@id_paciente, model: twnEntrega.xmlModel.tratamientos, id_financiador: twnEntrega.cboObraSocial.selectedItem.@id_financiador.toString(), nro_afiliado: twnEntrega.cboObraSocial.selectedItem.@nro_afiliado.toString()});
		}
	});
	
	twnEntrega.id_paciente = xmlModel.paciente.@id_paciente;
	
	twnEntrega.xmlObrasSociales = xmlModel.obras_sociales[0];
...
private function btnVerDatos_click(e:MouseEvent) : void {
	if (typeof autocompletePaciente.selectedItem == "xml") {
		...
		httpPaciente.addEventListener(ResultEvent.RESULT, function() : void {
			...
			btnAgregarItem.enabled = (xmlModel.paciente.@estado == "A");
			btnNuevaEntrega.enabled = (xmlModel.paciente.@estado == "A");
		});
		...
		btnModificarDatos.enabled = parentApplication.controlAcceso.tienePerfil("035003");
	} else {
		xmlModel = null;
